...
sub EditFieldRender {
    ...
    my $HTMLString = <<"EOF";
<input type="text" class="$FieldClass" id="$FieldName" name="$FieldName" title="$FieldLabelEscaped" value="$ValueEscaped" $MaxChars data-inputmask="$Mask"
 />
EOF

    if ($FieldConfig->{InputType} == 1) {
        $HTMLString = <<"EOF";
<textarea class="$FieldClass" id="$FieldName" name="$FieldName" title="$FieldLabelEscaped" $MaxChars data-inputmask="$Mask">$ValueEscaped</textarea>
EOF
    }
